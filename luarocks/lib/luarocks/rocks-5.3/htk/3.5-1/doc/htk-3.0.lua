...
local function build_constructor (field)
	return function (obj)
		...
		if not obj.class and Public.class_defaults[field] then
			addString (s, format (' class="%s"', Public.class_defaults[field]))
		end
...
local TEXTAREA = build_constructor ("TEXTAREA")

local TD = build_constructor ("TD")

local TH = build_constructor ("TH")

local SELECT = build_constructor ("SELECT")
...
function Public.CENTER (obj)
	obj.align = "center"
	return Public.DIV (obj)
end
...
function Public.MULTILINE (param)
	if param.value then
		param[1] = param.value
		param.value = nil
	end
...
Public.TEXTAREA = Public.MULTILINE
...
function Public.BUTTON (param)
	param.type = "button"
	return Public.INPUT (param)
end
...
function Public.PASSWORD (param)
	param.type = "password"
	return Public.INPUT (param)
end
...
function Public.RADIO_BUTTON (param)
	param.type = "radio"
	return Public.INPUT (param)
end
...
local function button_list (param, element)
	local item_separator = param.item_separator or Public.BR{}.."\n"
	...
	for i = 1, getn (list) do
		...
		if tli == "table" then
			...
			param[i] = Public.BOX {
				Public[element] (list[i]),
				item_separator,
			}
		elseif tli == "function" then	-- OTHER function!
			param[i] = Public.BOX {
				Public[element] (list[i] (param)),
				item_separator,
			}
		else
			param[i] = Public.BOX {
...
function Public.TOGGLE_LIST (param)
...
function Public.SELECT (param)
	local list = param.options
	for i = 1, getn (list) do
		local selected = check_selected (param, i)
		if type(list[i]) == "table" then
			param[i] = Public.OPTION {
				list[i][1];
				value = list[i].value,
...
function Public.COMPOUND (param)
...
function Public.DATE (param)
	...
	return Public.COMPOUND (param)
end

-----------------------------------------------------------------------------
function Public.RADIO_DATE (param)
end
...
function Public.TH (param)
	return TH (table_cell(param))
end
...
function Public.TD (param)
	return TD (table_cell(param))
end
...
function Public.DATE_FIELD (param)
...
function Public.FILE_FIELD (param)
...
function Public.PASSWORD_FIELD (param)
...
function Public.RADIO_FIELD (param)
...
function Public.SELECT_FIELD (param)
...
function Public.TEXT_FIELD (param)
...
function Public.TEXTAREA_FIELD (param)
	return input_field (param, "MULTILINE")
end

Public.MULTILINE_FIELD = Public.TEXTAREA_FIELD

-----------------------------------------------------------------------------
-- @param param Table with object description.
-- @return String with HTML representation of the object.

function Public.TOGGLE_FIELD (param)
...
function Public.OTHER (param)
	return function (obj)
		return {
			Public.BOX {
				param[1],
				param.type {
					name = obj.name.."__other_",
					value = (not checked (obj)) and obj.value,
				}
			}
			;
			name = obj.name,
