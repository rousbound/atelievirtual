	...
	return function (obj)
		...
		for i, v in obj do
			...
			elseif i ~= "separator" then
				...
				%I.addString (s, format (' %s="%s"', i, v))
			end
		end
		if not obj.class and %E.class_defaults[%field] then
			--s = format ('%s class="%s"', s, %E.class_defaults[%field])
			%I.addString (s, format (' class="%s"', %E.class_defaults[%field]))
		end
		--s = format ('<%s%s>%s', %field, s, separator)
		%I.addString (s, '>'..separator)
		local n = getn (contain)
		for i = 1, n do
			--s = format ('%s%s%s', s, contain[i], separator)
			%I.addString (s, contain[i])
			%I.addString (s, separator)
		end
		...
		return {
			%E.BOX {
				...
				%param.type {
					...
					value = (not %I.checked (obj)) and obj.value,
				}
			}
			;
			name = obj.name,
			value = %param.value,
			checked = (not %I.checked (obj)) and obj.value,
		}
	end
end
